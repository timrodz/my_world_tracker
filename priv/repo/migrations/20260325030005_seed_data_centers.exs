  ...
  def up do
    ...
    rows =
      Enum.map(@data_centers, fn dc ->
        Map.merge(dc, %{inserted_at: now, updated_at: now})
      end)

    repo().insert_all("data_centers", rows, on_conflict: :nothing)
  end

  def down do
    names = Enum.map(@data_centers, & &1.name)
    repo().delete_all(from(dc in "data_centers", where: dc.name in ^names))
  end
end
